...
def _resolve_world(context):
    ...
    candidates = [
        os.path.join(get_package_share_directory('puzzlebot_nav'),
                     'worlds', world_name),
...
def _fix_urdf_for_gazebo_harmonic(urdf: str) -> str:
    ...
    Root cause: URDF <material name="yellow"> tags get converted to Ogre1
    fixed-function materials (e.g. "Default/TransGreen") that Ogre2 rejects
    with "Fixed Function pipeline is no longer allowed". This silently drops
    the visual, making the robot invisible.
...
def generate_launch_description():
    declare_world = DeclareLaunchArgument(
        ...
        description='Archivo .world (busca en puzzlebot_nav y luego en puzzlebot_gazebo)')
