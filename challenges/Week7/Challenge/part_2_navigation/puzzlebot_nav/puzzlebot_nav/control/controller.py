"""
Nodo: controller  —  PID de posición (Task 1)
==============================================
Mini Challenge 4 · Manchester Robotics

ROL EN EL RETO:
    Mueve al robot por una trayectoria poligonal mientras Localisation
    propaga Σ_k. Esto permite VER la elipse creciendo en RViz a lo largo
    del recorrido — la demostración visual del Task 1.

ESTRATEGIA DE CONTROL:
    Angular (PID completo): corrige el error de orientación hacia la meta.
    Lineal  (P puro):       proporcional a la distancia restante.
    "Alinear primero":  si |error_angular| > 0.2 rad → v = 0 (gira en sitio).
                        Esto produce esquinas limpias en cuadrado/triángulo.

ECUACIONES:
    error_dist = √((tx−x)² + (ty−y)²)
    error_ang  = atan2(ty−y, tx−x) − θ      [normalizado a (−π, π)]
    ω = kp_w·err_ang + ki_w·∫err_ang·dt + kd_w·d(err_ang)/dt
    v = kv·err_dist          (solo si |err_ang| ≤ 0.2 rad)
...
CONTRATO ROS:
    ...
    Publica:    cmd_vel    (geometry_msgs/Twist)
                next_point (std_msgs/Bool)  — True una sola vez al llegar
"""
...
class PositionController(Node):
    ...
    def _target_callback(self, msg: Point):
        ...
        Importante: en modo reactivo (Bug) llegan setpoints cada 100 ms con
        cambios pequeños. Si reseteáramos el integrador y prev_ang_err en
        CADA mensaje, el derivativo escupiría picos y el integrador nunca
        acumularía. Solo reseteamos cuando el cambio de target es grande
        (> 0.5 m), interpretado como "nuevo waypoint, no refinamiento".
        """
        try:
            tx_prev = self.get_parameter('target_x').value
            ty_prev = self.get_parameter('target_y').value
            big_jump = math.hypot(msg.x - tx_prev, msg.y - ty_prev) > 0.5
        except Exception:
            big_jump = True
        ...
        if big_jump:
            ...
            self.get_logger().info(
                f'Nuevo objetivo (jump): x={msg.x:.3f}, y={msg.y:.3f}')
    ...
    def _control_loop(self):
        ...
        if dist > tol:
            ...
            # 3. Velocidad lineal — gradiente suave en lugar de "v=0 si mal alineado".
            #    El umbral duro a 0.2 rad funcionaba para waypoints estáticos
            #    pero rompe con setpoints reactivos (Bug): cada tick recibe un
            #    target ligeramente distinto y el robot solo gira sin avanzar.
            #    Aquí escalamos v con un factor cos² del error angular:
            #        ang_err = 0       → v = kv·dist  (avance pleno)
            #        ang_err = 60°     → v ≈ 0.25·kv·dist
            #        ang_err = 90°     → v = 0
            #    Esto permite avanzar mientras se corrige la orientación, sin
            #    sacrificar precisión cuando el robot está mirando al revés.
            align = max(0.0, math.cos(ang_err))   # 1 alineado, 0 a ±90°
            v = kv * dist * (align * align)
